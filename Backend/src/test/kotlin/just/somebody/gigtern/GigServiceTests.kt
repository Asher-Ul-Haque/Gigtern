...
class GigServiceTest {
	...
	@BeforeEach
	fun setUp() {
		gigService = GigService(gigRepository, employerRepository)
	}
	...
	fun `createGig should successfully create and return a gig DTO`() {
		// Mock dependencies
		`when`(employerRepository.findByUser(MOCK_EMPLOYER_USER)).thenReturn(MOCK_EMPLOYER_PROFILE)
		`when`(gigRepository.save(any(GigEntity::class.java))).thenReturn(MOCK_GIG_ENTITY)
		...
		verify(gigRepository, times(1)).save(any(GigEntity::class.java))
	}
	...
	fun `createGig should throw AuthorizationException if user is not an EMPLOYER`() {
		...
		verify(employerRepository, never()).findByUser(any())
	...
	fun `createGig should throw ResourceNotFoundException if employer profile is missing`() {
		// Mock dependencies
		`when`(employerRepository.findByUser(MOCK_EMPLOYER_USER)).thenReturn(null)
	...
	fun `getAllGigs should return a list of all gigs`() {
		...
		`when`(gigRepository.findAll()).thenReturn(allGigs)
	...
	fun `getAllGigs should return an empty list if no gigs exist`() {
		`when`(gigRepository.findAll()).thenReturn(emptyList())
	...
	fun `getMyGigs should return a list of gigs posted by the authenticated employer`() {
		...
		// Mock dependencies
		`when`(employerRepository.findByUser(MOCK_EMPLOYER_USER)).thenReturn(MOCK_EMPLOYER_PROFILE)
		`when`(gigRepository.findAllByEmployerId(MOCK_EMPLOYER_PROFILE.id!!)).thenReturn(myGigs)
	...
	fun `getMyGigs should throw AuthorizationException if user is not an EMPLOYER`() {
		...
		verify(gigRepository, never()).findAllByEmployerId(any())
	}
}